...
import { getDemoMode, getDemoStorage } from '@/lib/demo-session';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setDemoTourDismissed(storage?.getItem('poolops-demo-tour-dismissed') === 'true');
  }, []);
  ...
  useEffect(() => {
    ...
    const checklistDismissed = storage?.getItem('poolapp-checklist-dismissed');
    ...
    const justCompleted = storage?.getItem('poolapp-just-completed-onboarding');
    if (justCompleted === 'true') {
      setIsNewUser(true);
      storage?.removeItem('poolapp-just-completed-onboarding');
    }
  }, []);
    ...
    <div className="min-h-screen pb-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-amber-800">
                Changes persist only in this tab. Close the tab to reset. Use “Reset Demo” in the header any time.
              </p>
            </div>
            <Link
              href="/tech/route"
              className="shrink-0 px-3 py-2 rounded-lg bg-amber-200 text-amber-900 text-sm font-medium hover:bg-amber-300 transition-colors"
            >
              Try Tech App →
            </Link>
          </div>
      ...
      {isDemoMode && !demoTourDismissed && (
        <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Quick Demo Tour (2 min)</h3>
              <p className="text-sm text-slate-600 mt-1">Explore the core flows in order:</p>
              <div className="mt-3 grid gap-2 text-sm text-slate-700">
                <div>1. Optimize routes → <Link className="text-blue-600 hover:underline" href="/routes">Routes</Link></div>
                <div>2. Schedule a job → <Link className="text-blue-600 hover:underline" href="/schedule">Schedule</Link></div>
                <div>3. Complete a stop → <Link className="text-blue-600 hover:underline" href="/tech/route">Tech App</Link></div>
              </div>
            </div>
            <button
              onClick={() => {
                const storage = getDemoStorage();
                storage?.setItem('poolops-demo-tour-dismissed', 'true');
